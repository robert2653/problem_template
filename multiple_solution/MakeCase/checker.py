...
def user_output(inputFile, userFile, user_executable):
    with open(inputFile, 'r') as f_in:
        ...
        process = subprocess.run([user_executable], input=stdin_content.encode(), capture_output=True)
...
def testing(userCPP):
    ...
    user_name = os.path.splitext(os.path.basename(userCPP))[0]
    user_executable = os.path.join(USER_EXECUTION_FOLDER, user_name + '.out')
    # 編譯 userCPP
    ...
    for input_file in sample_cases:
        ...
        user_output(input_file, USER_FOLDER + case_name + '.ans', user_executable)
